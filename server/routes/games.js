...
router.post('/new', async (req, res) => {
  try {
    ...
    // Generar puzzle usando el generador del SERVIDOR
    ...
    // NO enviar la solución al cliente por seguridad
    res.json({
      success: true,
      gameId: game._id,
      puzzle: game.puzzle,
...
router.post('/:id/move', async (req, res) => {
  try {
    ...
    if (notesMode) {
      ...
      completed = checkCompletion(game.userBoard, game.solution);
      if (completed) {
        game.completed = true;
    ...
    res.json({
      ...
      progress: getProgress(game.userBoard),
...
router.post('/:id/hint', async (req, res) => {
  try {
    ...
    const completed = checkCompletion(game.userBoard, game.solution);
    if (completed) {
      game.completed = true;
      game.completedAt = new Date();
      game.time = Math.floor((game.completedAt - game.createdAt) / 1000);
...
function checkCompletion(userBoard, solution) {
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
...
function getProgress(userBoard) {
  const totalCells = 81;
  const filledCells = userBoard.flat().filter(cell => cell !== 0).length;
